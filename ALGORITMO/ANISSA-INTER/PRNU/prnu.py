...
def wam9(C, nmom=9):
    ...
    M0 = C - lawmlNSmartC(C) ##Wavelets David en C
    R = numpy.mean(M0)
    ...
    #New FOR who check if the return number of numpy is a Infinit or Not Number the old one has the problem of never check that
    arr = []
    for k in range(1, nmom + 1):
        number = numpy.sum(abs(M) ** k) / M.size
...
def getAverageNoise2(path, crop, wavelet, level, mode):
    ...
    for f in os.listdir(path):
        if os.path.splitext(f)[1].lower() in EXTS:
            ...
            if x==CROP_X_SIZE and y==CROP_X_SIZE:
...
def substractDarkFrame(image, darkFrameImage):
    cleanImage = ImageChops.difference(image, darkFrameImage)
    return cleanImage
...
def zeroMean2(image):
    ...
    for c in xrange(z): #by each color band
        ...
        for j in xrange(y):
                ...
                for i in xrange(x):
                    imageData[c, j, i] = max(0, imageData[c, j, i] - cj)
    outputImage = array2image(imageData)  
    return outputImage
...
def checkImagesSize(path, crop):
    ...
    for f in os.listdir(path):
        if os.path.splitext(f)[1].lower() in EXTS:
            ...
            x,y = im.size
            if x<CROP_X_SIZE or y<CROP_X_SIZE:
                images.append(f)
    if len(images) > 0:
