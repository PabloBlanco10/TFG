...
void submatrix(PyArrayObject* matriz, int N, int i, int j,double*** subMatriz, int *lenghtSubmatrizX, int *lenghtSubmatrizY) 
{
	...
	/*Crea una submatriz cuadrada de tamaño N de la matriz "matriz" centrada en i,j devolviendola en "subMatriz". Puede ser que la submatriz
	...
	Se reseva memoria dinámicamente para la submatriz asi que HAY QUE LIBERARLA.
	*/
	N=N/2;
...
{
	/*Esta función calcula var = mean(abs(x - x.mean())**2) donde mean() es la media es decir la suma de las
	componentes dividido por el número de elementos*/
	  ...
	  //resto la media a cada posición del vector, hago el valor absoluto y lo elevo a la 2.
	  for(i=0;i<lenghtMatrizX;i++){  //relleno del resultado                               
            for(j=0;j< lenghtMatrizY;j++){
...
{
	/*Las dos matrices tienen el mismo tamaño mirar documentación web numpy.fmin
...
static PyObject *waveletsC_lawmlNC(PyObject *self,PyObject *args)
{
	...
	Tiene 6 parámetros. Primero la imagen con ruido, segundo y tercero arrays auxiliares inicializados a 0 del tamaño de la imagen con ruido, 
...
static PyMethodDef waveletsCMethods[] = {
 {"lawmlNC", waveletsC_lawmlNC, METH_VARARGS, "Extracción de ruido de imagen"},
 {NULL, NULL, 0, NULL}
 };
